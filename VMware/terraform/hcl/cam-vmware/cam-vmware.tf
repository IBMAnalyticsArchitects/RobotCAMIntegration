...
#  folder = "${var.vm_folder}"
  ...
  clone {
    ...
    customize {
      ...
      network_interface {
        ipv4_address = "${local.vm_ipv4_address_base }.${local.vm_ipv4_address_start + 3 + var.num_workers + count.index }"
        ipv4_netmask = "${ var.vm_ipv4_prefix_length }"
      }
    ipv4_gateway = "${var.vm_ipv4_gateway}"
    }
...
resource "vsphere_virtual_machine" "icpnfs" {
  ...
  clone {
    ...
    customize {
      ...
      network_interface {
        ipv4_address = "${local.vm_ipv4_address_base }.${local.vm_ipv4_address_start + count.index + 4 + var.num_workers }"
        ipv4_netmask = "${ var.vm_ipv4_prefix_length }"
      }
    ipv4_gateway = "${var.vm_ipv4_gateway}"
    }
