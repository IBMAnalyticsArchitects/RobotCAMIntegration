...
variable "icp_cluster_vip" {
  description = "Portable Virtual IP for ICP Cluster"
}

variable "icp_proxy_vip" {
  description = "Portable Virtual IP for ICP Proxy"
}
...
resource "ibm_compute_vm_instance" "icpidm" {
  count="1"
  hostname = "${var.vm_name_prefix}-icpidm"
  os_reference_code        = "REDHAT_7_64"
  domain                   = "${var.vm_domain}"
  datacenter               = "${var.datacenter}"
  private_vlan_id          = "${data.ibm_network_vlan.cluster_vlan.id}"
...
###########################################################################################################################################################
# ICP Boot
resource "ibm_compute_vm_instance" "icpboot" {
  count="1"
  hostname = "${var.vm_name_prefix}-icpboot"
  os_reference_code        = "REDHAT_7_64"
  domain                   = "${var.vm_domain}"
  datacenter               = "${var.datacenter}"
  private_vlan_id          = "${data.ibm_network_vlan.cluster_vlan.id}"
...
resource "ibm_compute_vm_instance" "icpnfs" {
  count="1"
  hostname = "${var.vm_name_prefix}-icpnfs"
  os_reference_code        = "REDHAT_7_64"
  domain                   = "${var.vm_domain}"
  datacenter               = "${var.datacenter}"
  private_vlan_id          = "${data.ibm_network_vlan.cluster_vlan.id}"
...
resource "null_resource" "start_install" {
  ...
  depends_on = [ 
  	...
  	"ibm_compute_vm_instance.icpboot",  
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo  export cam_icpboot_ip=${join(",",ibm_compute_vm_instance.icpboot.*.ipv4_address_private)} >> /opt/monkey_cam_vars.txt",
      "echo  export cam_icpboot_name=${join(",",ibm_compute_vm_instance.icpboot.*.hostname)} >> /opt/monkey_cam_vars.txt",    
      ...
      "echo  export cam_icp_cluster_vip=${var.icp_cluster_vip} >> /opt/monkey_cam_vars.txt",
      "echo  export cam_icp_proxy_vip=${var.icp_proxy_vip} >> /opt/monkey_cam_vars.txt",
